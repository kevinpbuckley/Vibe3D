...
`/Game/Developers/Vibe3DModelingTests` and removed in the last section.

For a no-agent check of the same surface, run `Content/Skills/modeling/scripts/exercise_all.txt`
(calls every function and prints `RESULT: PASS`), or the automation tests `Vibe3D.Modeling.*`
(console: `Automation RunTests Vibe3D.Modeling`).

## Prerequisites

- UE 5.8 editor with Vibe3D enabled and the MCP server running.
...
## Prompt 10.1
Save the crate from section 4 as `/Game/Developers/Vibe3DModelingTests/SM_Crate` with collision enabled and Nanite off; then save again to the same path to prove replace works, and load it back into a new handle.
...
## Prompt 10.4
Bake a 64 px tangent-space normal map and ambient occlusion from the noisy sphere onto the UV'd box into `/Game/Developers/Vibe3DModelingTests`, and report the texture asset paths.
...
## Prompt 11.1
Find a SkeletalMesh in the project, load its LOD 0 into a handle, remesh it to 5000 triangles, transfer bone weights back from the source asset, smooth them against its skeleton, and save as `/Game/Developers/Vibe3DModelingTests/SK_Remeshed` bound to the same skeleton.
...
## Prompt 13.1
Delete the `ModelingTestCrate` actor, delete everything under `/Game/Developers/Vibe3DModelingTests`, and release all modeling handles. Confirm `list_meshes` is empty.
